  ...
  <div class="row justify-content-center">
    <div class="col-12 mb-3">
      <div class="sorting-container text-center">
        <a href="<?= base_url('ulasan/list/'.$id); ?>" class="btn <?= ($rating == 0)?'btn-primary':'btn-outline-primary' ?> mb-2 mr-3">Semua</a>
        <?php for($i=1; $i<=5; $i++): ?>
          <a href="<?= base_url('ulasan/list/'.$id.'?rating='.$i); ?>" class="btn <?= ($i == $rating)?'btn-primary':'btn-outline-primary' ?> mb-2 mr-3">
            <?php for($s=1; $s<=$i; $s++): ?>
